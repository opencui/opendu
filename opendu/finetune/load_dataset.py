...
from opendu.finetune.commons import (MergedDatasetFactory, FtDatasetFactory)


def load_merged_factories(paths, mode, prompt)-> MergedDatasetFactory:
    factories = []
    for path in paths:
...
def load_training_dataset(training_mode, debug=False):
    ...
    if training_mode == "yni":
        ...
        prompt_type = RauConfig.get().yni_prompt
        print(f"load yni datasets: {datasets}")
        factory = load_merged_factories(datasets, training_mode, prompt_type)
