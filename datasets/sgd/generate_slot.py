...
class IntentTemplate:
    ...
    def generate_equal_sample_fixed(self, expression, fixed_slot_name, fixed_slot_val, masked_len=0):
        ...
        expression_template = expression.template
        for slot_name, slot_vals in self.slot_dict.items():
            if '|||'+slot_name+'|||' in expression_template and  \
                    expression_template.find('|||'+slot_name+'|||') != expression_template.find('|||'+fixed_slot_name+'|||'):
                expression_template = expression_template.replace('|||'+slot_name+'|||', list(slot_vals)[random.randint(0, len(slot_vals)-1)])
...
def judge_legal_loc(utterance,pattern):
    ...
    for idx, (start, end) in enumerate(span):
        if (start == 0 or utterance[start - 1] == ' ' or
            utterance[start - 1] in '!"#$%&\'()*+,-./;<=>?@[\\]^_`{|}~') and \
                (end == len(utterance) - 1 or
                 utterance[end + 1] == ' ' or
                 utterance[end + 1] in '!"#$%&\'()*+,-./;<=>?@[\\]^_`{|}~'):
            return True
                                ...
                                if frame['service'][-1] == '1':
                                    ...
                                    for slot_name, slot_val_list in frame['state']['slot_values'].items():
                                        for idx in range(len(slot_val_list)):
                                            slot_val_list[idx] = slot_val_list[idx].lower()
                                    ...
                                    # the slot that would be used in the "slot_values"  is the new slot name,
                                    # or the slot values has been changed
                                    for slot_name, slot_val_list in frame['state']['slot_values'].items():
                                    ...
                                    for slot_name, slot_val_list in frame['state']['slot_values'].items():
                                        if slot_name in cur_slot_name-pre_slot_name:
                                            for slot_val in slot_val_list:
                                                # if the slot val in the utterance locates more than 1 ,then
                                                # we could not judge which corresponds to it ,so we add
                                                # it into the vague list
                                    ...
                                    for slot_val,slot_name_list in new_slot_val2slot_name.items():
                                        # if in the new slot list exists one value corresponds to more than one slot
                                        # name, we could not judge the which it belongs to ,so we  need to ignore it
                                    ...
                                    for slot_val, slot_name in single_dict:
                                        if len(judge_single_word(turn['utterance'], slot_val)) == 1 and \
                                            (slot_name not in slots) and  \
                                            ((slot_name in cur_slot_name-pre_slot_name and
                                              slot_name not in vague_slot_names) or
                                            (slot_name in cur_slot_name & pre_slot_name and
                                             slot_val in set(frame['state']['slot_values'][slot_name]) - set(pre_slot[slot_name]))):
                                            is_covered = False
                                            ...
                                            for _, pre_slot_val_list in slots.items():
                                                for pre_slot_str,pre_span in pre_slot_val_list.items():
                                                    if slot_val in pre_slot_str and \
                                                            start_position >= pre_span[0] and\
                                                            end_position <= pre_span[1]:
                                                        is_covered = True
                                            if is_covered is False:
                                                slots[slot_name][slot_val] = (start_position, end_position)
                                                string_list.append((start_position, end_position))
                                    # the slot val that been flagged in the slots should be used
                                    vague_slot_names = vague_slot_names-set(old_slots.keys())
                                    ...
                                    expression = Expression(
                                        ...
                                        slots,
                                        frame['service'],
                                        string_list, vague_slot_names, dialogue['dialogue_id'])
...
class SlotExample:
    def __init__(self, quintuple):
        self.source = quintuple[0]
        self.utterance = quintuple[1]
        self.slot_name = quintuple[2]
        self.slot_description = None
        self.candidates = None  # this should include all the occurrences of the slot type.
        ...
        for expression in expressions:
            ...
            for slot in slots_dict[intent]:
                if slot in expression.slots:
                    for slot_val, span in expression.slots[slot].items():
                        ...
                        examp = [service_intent, equal_sent.lower(), " ".join(slot.split('_')), 1, (new_start, new_end)]
                        partition = dataset_type(FLAGS.training_percentage, FLAGS.dev_percentage)
                        examples[partition].append(SlotExample(examp))
                        ...
                        examp = [service_intent, equal_sent, " ".join(slot.split('_')), 1, (new_start, new_end)]
                        partition = dataset_type(FLAGS.training_percentage, FLAGS.dev_percentage)
                        examples[partition].append(SlotExample(examp))
            ...
            for slot in slots_dict[intent]:
                if slot not in expression.slots and slot not in vague_slot_names:
                    ...
                    examp = [service_intent, equal_sent.lower(), " ".join(slot.split('_')), 0, (0, 0)]
                    partition = dataset_type(FLAGS.training_percentage, FLAGS.dev_percentage)
                    examples[partition].append(SlotExample(examp))
                    neg_count += 1
...
def save(slot_examples, path):
    ...
    for key, examples in slot_examples.items():
        ...
        with open(f"{path}_slot.json", 'w') as f:
...
def main(_):
    slots_meta = generate_slotindex(FLAGS.input)

    slot_expressions, intent_template_dict = load_add_slot_value(FLAGS.input, slots_meta)
    ...
    slot_examples = build_slot_examples(slot_expressions, slots_meta, intent_template_dict)
